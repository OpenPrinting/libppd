...
//
// 'ppdArrayAdd()' - Add a string to an array.
//
// Note: The array MUST be created using the @link ppdArrayNew@
// function. Duplicate strings ARE added. If the string pointer "s" is NULL
// or the empty string, no strings are added to the array.
//

int					// O - 1 on success, 0 on failure
ppdArrayAdd(cups_array_t *a,	// I - Array
		    const char   *s)	// I - String
{
  ...
  DEBUG_printf(("_ppdArrayAddStrings(a=%p, s=\"%s\")",
		(void *)a, s));
...
//
// 'ppdArrayNew()' - Create a new array of strings.
//
// Note: The array automatically manages copies of the strings passed. If the
// string pointer "s" is NULL or the empty string, no strings are added to the
// newly created array.
...
cups_array_t *				// O - Array
ppdArrayNew(const char *s)	// I - Delimited strings or NULL
{
  cups_array_t	*a;			// Array


  if ((a = cupsArrayNew3(NULL, NULL, NULL, 0,
                         (cups_acopy_func_t)_ppdStrAlloc,
			 (cups_afree_func_t)_ppdStrFree)) != NULL)
    ppdArrayAdd(a, s);
