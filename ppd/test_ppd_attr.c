...
main(void)
{
  ...
  // T09 — *ModelName: "Acme AttrTest" is stored as an attribute; with a
  //       NULL spec the first (only) match is returned and its dequoted value
  //       must equal the declared string.
  testBegin("ppdFindAttr(ppd, \"ModelName\", NULL) value == \"Acme AttrTest\"");
  attr = ppdFindAttr(ppd, "ModelName", NULL);
  testEndMessage(attr != NULL && attr->value != NULL &&
                 !strcmp(attr->value, "Acme AttrTest"),
                 "got \"%s\"", (attr && attr->value) ? attr->value : "(null)");

  // T10 — *Manufacturer: "Acme" → value "Acme".
  testBegin("ppdFindAttr(ppd, \"Manufacturer\", NULL) value == \"Acme\"");
  attr = ppdFindAttr(ppd, "Manufacturer", NULL);
  testEndMessage(attr != NULL && attr->value != NULL &&
                 !strcmp(attr->value, "Acme"),
                 "got \"%s\"", (attr && attr->value) ? attr->value : "(null)");

  // T11 — *NickName: "Acme AttrTest, 1.0" → value preserved verbatim.
  testBegin("ppdFindAttr(ppd, \"NickName\", NULL) value == \"Acme AttrTest, 1.0\"");
  attr = ppdFindAttr(ppd, "NickName", NULL);
  testEndMessage(attr != NULL && attr->value != NULL &&
                 !strcmp(attr->value, "Acme AttrTest, 1.0"),
                 "got \"%s\"", (attr && attr->value) ? attr->value : "(null)");
